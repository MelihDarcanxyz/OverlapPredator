...
from lib.benchmark_utils import get_correspondences, to_o3d_pcd
...
class KITTIDataset(Dataset):
    ...
    def prepare_kitti_ply(self, split):
        ...
        for dirname in subset_names:
            ...
            assert len(fnames) > 0, (
                f"Make sure that the path {self.root} has data {dirname}"
            )
    ...
    def __getitem__(self, idx):
        ...
        # Get matches
        matching_inds = get_correspondences(
            pcd0, pcd1, tsfm, self.matching_search_voxel_size
        )
        ...
        if self.data_augmentation:
            # add gaussian noise
            src_pcd_input += (
                np.random.rand(src_pcd_input.shape[0], 3) - 0.5
            ) * self.augment_noise
            tgt_pcd_input += (
                np.random.rand(tgt_pcd_input.shape[0], 3) - 0.5
            ) * self.augment_noise
            ...
            scale = (
                ...
                + (self.augment_scale_max - self.augment_scale_min) * random.random()
            )
            src_pcd_input = src_pcd_input * scale
            tgt_pcd_input = tgt_pcd_input * scale

            # shift the pcd
            shift_src = np.random.uniform(
                -self.augment_shift_range, self.augment_shift_range, 3
            )
            shift_tgt = np.random.uniform(
                -self.augment_shift_range, self.augment_shift_range, 3
            )
    ...
    def get_position_transform(self, pos0, pos1, invert=False):
        ...
        return (
            np.dot(T1, np.linalg.inv(T0)).T
            if not invert
            else np.dot(np.linalg.inv(T1), T0).T
        )
