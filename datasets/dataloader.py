...
def batch_grid_subsampling_kpconv(
    ...
    if (features is None) and (labels is None):
        ...
        return (
            torch.from_numpy(s_points),
            torch.from_numpy(s_len),
            torch.from_numpy(s_features),
        )
...
def collate_fn_descriptor(list_data, config, neighborhood_limits):
    ...
    for block_i, block in enumerate(config.architecture):
        ...
        if not ("pool" in block or "strided" in block):
            layer_blocks += [block]
            if (
                block_i < len(config.architecture) - 1
                and "upsample" not in config.architecture[block_i + 1]
            ):
                continue
...
def calibrate_neighbors(
    dataset, config, collate_fn, keep_ratio=0.8, samples_threshold=2000
):
    timer = Timer()
...
def get_dataloader(
    dataset, batch_size=1, num_workers=4, shuffle=True, neighborhood_limits=None
):
