...
def benchmark_predator(
    feats_scores,
    n_points,
    exp_dir,
    whichbenchmark,
    sample_method,
    ransac_with_mutual=False,
    ...
    for eachfile in tqdm(feats_scores):
        ...
        tsfm_est.append(
            ransac_pose_estimation(
                src_pcd, tgt_pcd, src_feats, tgt_feats, mutual=ransac_with_mutual
            )
        )

        ########################################
        # 4. calculate inlier ratios
        inlier_ratio_results = get_inlier_ratio(
            src_pcd, tgt_pcd, src_feats, tgt_feats, rot, trans
        )

        results["w_mutual"]["inlier_ratios"].append(
            inlier_ratio_results["w"]["inlier_ratio"]
        )
        results["w_mutual"]["distances"].append(inlier_ratio_results["w"]["distance"])
        results["wo_mutual"]["inlier_ratios"].append(
            inlier_ratio_results["wo"]["inlier_ratio"]
        )
    ...
    for key in ["w_mutual", "wo_mutual"]:
        ...
        with open(os.path.join(exp_dir, "result"), "a") as f:
            f.write(
                f"Inlier ratio {key}: {np.mean(inliers):.3f} : +- {np.std(inliers):.3f}\n"
            )
            f.write(
                f"Feature match recall {key}: {np.mean(fmrs):.3f} : +- {np.std(fmrs):.3f}\n"
            )
        f.close()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=None,
        type=str,
        help="path to precomputed features and scores",
    )
    parser.add_argument(
    ...
    benchmark_predator(
        feats_scores, args.n_points, args.exp_dir, args.benchmark, args.sampling
    )
